...
__author__ = "Jasmijn Baaijens, Xiongbin Kang, Xiao Luo"
# __license__ = "GPL"
...
#    advanced.add_argument('--no_filtering', dest='filtering', action='store_false', help='disable kallisto-based filtering of contigs')
    ...
    #extra parameters tuning
    advanced.add_argument('--edge1', dest='edge1', type=float, default=0.95, help='overlap score in the first iteration')
    advanced.add_argument('--edge2', dest='edge2', type=float, default=1.0, help='overlap score in the next iterations')
...
Author: %s
    ...
    if only_strain_count:
        ...
        if args.reference:
            ...
            run_strain_count(args.reference, final_contig_file, base_path3)
            FNULL.close()
...
#    if preprocessing:
    ...
    # First step (Preprocessing): rename and reorganize reads
    if preprocessing:
        print("*******************")
        ...
        subprocess.check_call("%s/rename_fas.py --in tmp.fastq --out assembly/s_p1_p2.fastq" % (base_path3), shell=True)
    ...
    if compute_overlaps:
        ...
        # find all overlaps
        if denovo:
            preprocessing_denovo(min_overlap_len_EC, args.sfo_err, args.threads, base_path3)
        os.chdir('..')
        print("- Done!")
        sys.stdout.flush()
    elif args.assembly:
    ...
    if args.assembly:
        ...
        build_threshold_table(readlen, intseg, stddev, hcov, base_path3)
        run_savage_assembly(EC, args.min_overlap_len, min_overlap_len_EC, args.min_clique_size, min_read_len, max_tip_len, branch_reduction, error_rate, args.threads, original_readcount, diploid, base_path,args.edge1,args.edge2, verbose)
        os.chdir('..')
        if os.path.exists('assembly/singles.fastq'):
            subprocess.check_call("%s/fastq2fasta.py assembly/singles.fastq contigs.fasta" % base_path3, shell=True)
    ...
    if args.diploid:
        ...
        os.chdir('diploid')
        reversals = True
        run_sfo('../contigs.fasta', args.merge_contigs, base_path3, min_overlap_len, args.threads, contig_count, paired_count, "original_overlaps.txt", reversals)
        sys.stdout.flush()
        # run SAVAGE
        ...
        branch_reduction = [args.hap_cov, SE_count, int(PE_count/2)]
        build_threshold_table(readlen, intseg, stddev, hcov, base_path3)
        run_savage_assembly(EC, min_overlap_len, min_overlap_len, min_clique_size, min_read_len, max_tip_len, branch_reduction, error_rate, args.threads, original_readcount, diploid, base_path3, args.edge1,args.edge2,verbose)
        os.chdir('..')
        if os.path.exists('diploid/singles.fastq'):
            subprocess.check_call("%s/fastq2fasta.py diploid/singles.fastq contigs_diploid.fasta" % base_path3, shell=True)
            final_contig_file = "contigs_diploid.fasta"
            print("Done!")
        else:
...
def preprocessing_denovo(min_overlap_len, sfo_err, threads, base_path3):
    sys.stdout.flush()
    # prepare fasta
    subprocess.check_call("%s/fastq2fasta.py s_p1_p2.fastq s_p1_p2.fasta" % base_path3, shell=True)
    read_count = int(file_len('s_p1_p2.fastq')/4.0)
    # run SFO
    print("Running minimap2 overlaps")
    sys.stdout.flush()
    ...
    try:
        identity = 0.98
        outdir='.'
        sfo = outdir + '/sfoverlaps.out'
        overlaps = outdir + '/original_overlaps.txt'
        paf = outdir+ '/overlaps.paf'
        o=1 #overhang length
        r=0.8
        filter_inline = 'python ' + base_path3 + '/filter_ovlp_inline.py {} {} {} {} '. \
                   format(min_overlap_len, identity, o, r)
                    
        minimap = "/prj/metastrain/software/conda/bin/minimap2 --sr -X -c -k 21 -w 11 -s 60 -m 30 -n 2 -r 0 -A 4 -B 2 --end-bonus 100  " + \
                          "-t %s s_p1_p2.fasta s_p1_p2.fasta |cut -f 1-12| %s >%s" % (threads, filter_inline, paf)
        os.system(minimap)
        paf2sfo_cmd = 'python %s/minimap22sfo.py --in %s --out %s -m 0 -p 0' % (base_path3, paf, sfo)
        sfo2overlaps_cmd = 'python %s/sfo2overlaps.py --in %s --out %s --num_singles %d  --num_pairs 0' \
                               % (base_path3, sfo, overlaps, read_count)
  ...
  #      subprocess.check_call("rust-overlaps -i -r -w %d s_p1_p2.fasta sfoverlaps.out %f %d 1> /dev/null" % (threads, sfo_err, sfo_len), shell=True)
    ...
    sys.stdout.flush()
    subprocess.check_call("%s/sfo2overlaps.py --in sfoverlaps.out --out original_overlaps.txt --num_singles %d --num_pairs 0 1> /dev/null" % (base_path3, read_count), shell=True)
...
def run_sfo(fasta, sfo_err, base_path3, min_overlap_len, threads, s_count, p_count, overlaps_file, reversals):
#    print "sfo_err: ", sfo_err
    read_count = int(file_len(fasta)/2.0)
    if reversals:
        try:
            identity = 0.98
            outdir='.'
...
#            subprocess.check_call("rust-overlaps -i -r -w %d s_p1_p2.fasta sfoverlaps.out %f %d 1> /dev/null" % (threads, sfo_err, sfo_len), shell=True)
        ...
        try:
            ...
            try:
                ...
                print("rust-overlaps error; continuing.")
                subprocess.check_call("touch sfoverlaps.out", shell=True)
    subprocess.check_call("%s/sfo2overlaps.py --in sfoverlaps.out --out %s --num_singles %d --num_pairs %d 1> /dev/null" % (base_path3, overlaps_file, s_count, p_count), shell=True)
    subprocess.check_call("rm sfoverlaps.out", shell=True)
    return



def run_savage_assembly(EC, min_overlap_len, min_overlap_len_EC, min_clique_size, min_read_len, max_tip_len, branch_reduction, error_rate, threads, original_readcount, diploid, base_path3, edge1,edge2,verbose=False):
    ...
    if EC == "true":
        edge_threshold = edge1 #0.95 #0.97
        first_it = "true"
        cliques = "true"
...
def run_viralquasispecies(stats, fastq, overlaps, min_overlap_len, next_min_overlap, min_clique_size, edge_threshold, min_read_len, max_tip_len, first_it, cliques, EC, branch_reduction, error_rate, threads, original_readcount, diploid, verbose, final_it, remove_tips):
    ...
    selfpath = base_path
    viralquasispecies = base_path+"/bin/ViralQuasispecies"
    COPYFILES = False
    if EC=="true":
...
#    remove_tips = "true" if final_it and diploid=="true" else "false"
    ...
    if denovo:
        ...
        if s_count > 0:
            subprocess.check_call("%s/fastq2fasta.py singles.fastq singles.fasta" % base_path3, shell=True)
            sfo_err = 0
            reversals = True #if first_it=="true" else False # allow reversals only in the first two iterations
            run_sfo("singles.fasta", sfo_err, base_path3, next_min_overlap, threads, s_count, 0, "overlaps.txt", reversals)
...
def analyze_coverage():
    cov_counts = [0 for i in range(original_readcount)]
    max_cov = 0
...
def build_threshold_table(readlen, intseg, stddev, hcov, base_path3):
    ...
    # build a new table
    shell_command = "{}/min_ev_table.py".format(base_path3)
